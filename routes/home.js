...
router.get("/profile", (req, res) => {
    if (req.session.user && req.session.user.type == "student") {
        const qry1 = req.app.get("db").query(sql.select_students_with_id, [req.session.user.userid]);
        qry1.then(([students]) => {
        ...
        const qry2 = req.app.get("db").query(sql.select_teachers_with_id, [req.session.user.userid]);
        qry2.then(([teachers]) => {
...
router.get("/users", (req, res) => {
    if (req.session.user && req.session.user.type == "admin") {
        const qry = req.app.get("db").query(sql.select_all_users);
...
router.get("/user/:userid", (req, res) => {
    if (req.session.user && req.session.user.type == "admin") {
        if (vfv.verify_userid(req.params.userid)) {
            ...
            let v_user;
            const qry = req.app.get("db").query(sql.select_users_with_id, [req.params.userid]);
            qry.then(([users]) => {
                if (users.length > 0 && users[0]) {
                    v_user = users[0];
                    if (v_user.type == "student")
                        return req.app.get("db").query(sql.select_students_with_id, [req.params.userid]);
                    else if (v_user.type == "teacher")
                        return req.app.get("db").query(sql.select_teachers_with_id, [req.params.userid]);
                    else
...
router.post("/add-user", (req, res) => {
    if (req.session.user && req.session.user.type == "admin") {
        ...
        if (vfv.verify_password(req.body.password) && vfv.verify_type(req.body.type) && vfv.verify_active(req.body.active)
            ...
            let v_insertedUser;
            const qry = req.app.get("db").query(sql.insert_users_table, [req.body.password, req.body.type, req.body.active]);
            qry.then(([insertedUser]) => {
                v_insertedUser = insertedUser;
                if (req.body.type == "student")
                    return req.app.get("db").query(sql.insert_students_table,
                        [v_insertedUser.insertId, req.body.name, req.body.course, req.body.dob, req.body.email, req.body.phone, req.body.address]);
                else if (req.body.type == "teacher")
                    return req.app.get("db").query(sql.insert_teachers_table,
                        [v_insertedUser.insertId, req.body.name, req.body.course, req.body.dob, req.body.email, req.body.phone, req.body.address]);
...
const relocate_student_to_teacher_and_respond = (req, res, userid) => {
    let v_student;
    const qry = req.app.get("db").query(sql.select_students_with_id, [userid]);
    qry.then(([students]) => {
        if (students.length > 0 && students[0]) {
            v_student = students[0];
            return req.app.get("db").query(sql.delete_students_with_id, [v_student.userid]);
        } else redirecth.not_found(req, res, "users");
    }).then((result) => {
        if (!result) return;
        return req.app.get("db").query(sql.insert_teachers_table,
...
const relocate_teacher_to_student_and_respond = (req, res, userid) => {
    let v_teacher;
    const qry = req.app.get("db").query(sql.select_teachers_with_id, [userid]);
    qry.then(([teachers]) => {
        if (teachers.length > 0 && teachers[0]) {
            v_teacher = teachers[0];
            return req.app.get("db").query(sql.delete_teachers_with_id, [v_teacher.userid]);
        } else redirecth.not_found(req, res, "users");
    }).then((result) => {
        if (!result) return;
        return req.app.get("db").query(sql.insert_students_table,
...
router.post("/edit-user", (req, res) => {
    if (req.session.user && req.session.user.type == "admin") {
        if (vfv.verify_userid(req.body.userid)) {
            ...
            const qry = req.app.get("db").query(sql.select_users_with_id, [req.body.userid]);
            qry.then(([users]) => {
                if (users.length > 0 && users[0] && vfv.verify_type(users[0].type)) {
                    if (vfv.verify_password(req.body.password))
                        return req.app.get("db").query(sql.update_users_with_id("password"), [req.body.password, users[0].userid]);
                    else if (vfv.verify_active(req.body.active))
                        return req.app.get("db").query(sql.update_users_with_id("active"), [req.body.active, users[0].userid]);
                    else if (vfv.verify_name(req.body.name) && users[0].type == "student")
                        return req.app.get("db").query(sql.update_students_with_id("name"), [req.body.name, users[0].userid]);
                    else if (vfv.verify_name(req.body.name) && users[0].type == "teacher")
                        return req.app.get("db").query(sql.update_teachers_with_id("name"), [req.body.name, users[0].userid]);
                    else if (vfv.verify_course(req.body.course) && users[0].type == "student")
                        return req.app.get("db").query(sql.update_students_with_id("course"), [req.body.course.toLowerCase(), users[0].userid]);
                    else if (vfv.verify_course(req.body.course) && users[0].type == "teacher")
                        return req.app.get("db").query(sql.update_teachers_with_id("course"), [req.body.course.toLowerCase(), users[0].userid]);
                    else if (vfv.verify_dob(req.body.dob) && users[0].type == "student")
                        return req.app.get("db").query(sql.update_students_with_id("dob"), [req.body.dob, users[0].userid]);
                    else if (vfv.verify_dob(req.body.dob) && users[0].type == "teacher")
                        return req.app.get("db").query(sql.update_teachers_with_id("dob"), [req.body.dob, users[0].userid]);
                    else if (vfv.verify_email(req.body.email) && users[0].type == "student")
                        return req.app.get("db").query(sql.update_students_with_id("email"), [req.body.email.toLowerCase(), users[0].userid]);
                    else if (vfv.verify_email(req.body.email) && users[0].type == "teacher")
                        return req.app.get("db").query(sql.update_teachers_with_id("email"), [req.body.email.toLowerCase(), users[0].userid]);
                    else if (vfv.verify_phone(req.body.phone) && users[0].type == "student")
                        return req.app.get("db").query(sql.update_students_with_id("phone"), [req.body.phone, users[0].userid]);
                    else if (vfv.verify_phone(req.body.phone) && users[0].type == "teacher")
                        return req.app.get("db").query(sql.update_teachers_with_id("phone"), [req.body.phone, users[0].userid]);
                    else if (vfv.verify_address(req.body.address) && users[0].type == "student")
                        return req.app.get("db").query(sql.update_students_with_id("address"), [req.body.address, users[0].userid]);
                    else if (vfv.verify_address(req.body.address) && users[0].type == "teacher")
                        return req.app.get("db").query(sql.update_teachers_with_id("address"), [req.body.address, users[0].userid]);
                        ...
                        req.app.get("db").query(sql.update_users_with_id("type"), [req.body.type, users[0].userid])
...
router.post("/delete-users", (req, res) => {
    if (req.session.user && req.session.user.type == "admin") {
        if (vfv.verify(req.body.userid)) {
            req.body.userid = vfv.get_real_userid(req.body.userid);
            req.app.get("db").query(sql.select_users_with_id, [req.body.userid], (err, users) => {
                if (err) {
                    ...
                    if (users[0].type == "student") {
                        req.app.get("db").query(sql.delete_students_with_id, [users[0].userid], (err, qRes) => {
                            if (err) {
                                global.errorMsg = "System Error.";
                                ...
                                req.app.get("db").query(sql.delete_all_answer_papers_with_id, [users[0].userid], (err, aRes) => {
                                    if (err) {
                                        global.errorMsg = "System Error.";
                                        ...
                                        req.app.get("db").query(sql.delete_users_with_id, [users[0].userid], (err, uRes) => {
                                            if (err) {
                                                global.errorMsg = "System Error.";
                        ...
                        req.app.get("db").query(sql.delete_teachers_with_id, [users[0].userid], (err, qRes) => {
                            if (err) {
                                global.errorMsg = "System Error.";
            ...
            qry.then(([users]) => {
                if (users.length > 0 && users[0]) {
                    ...
                    if (vfv.verify_type(v_user.type) && v_user.type == "student")
                        return req.app.get("db").query(sql.delete_students_with_id, [v_user.userid]);
                    else if (vfv.verify_type(v_user.type) && v_user.type == "teacher")
                        return req.app.get("db").query(sql.delete_teachers_with_id, [v_user.userid]);
                    else redirecth.not_found(req, res, "users");
                } else redirecth.not_found(req, res, "users");
            }).catch(err => redirecth.system_error(req, res, err, "users"))
                .then((result) => {
                    if (!result) return [null];
                    return req.app.get("db").query(sql.select_all_answer_papers_with_id, [v_user.userid]);
                }).then(([aPapers]) => {
                    if (!aPapers) return;
                    for (let i = 0; i < aPapers.length; i++) {
                        fileh.delete_aPaper(req.app.get("db"), aPapers[i].question_filename, aPapers[i].userid);
                    }
                    return req.app.get("db").query(sql.delete_users_with_id, [v_user.userid]);
                }).then((result) => {
                    if (!result) return;
...
router.get("/question-papers", (req, res) => {
    if (req.session.user && req.session.user.type == "teacher") {
        const qry = req.app.get("db").query(sql.select_teachers_with_id, [req.session.user.userid]);
        qry.then(([teachers]) => {
            if (teachers.length > 0 && teachers[0])
                return req.app.get("db").query(sql.select_all_question_papers_with_course, [teachers[0].course]);
            else redirecth.permission_denied(req, res, null);
            return [null];
        }).then(([qPapers]) => {
            if (!qPapers) return;
    ...
    if (req.session.user && req.session.user.type == "teacher") {
        if (vfv.verify_Qname(req.params.Qname)) {
            ...
            qry.then(([teachers]) => {
                if (teachers.length > 0 && teachers[0]) {
                    v_teacher = teachers[0];
                    return req.app.get("db").query(sql.select_question_papers_with_name, [req.params.Qname]);
                ...
                if (qPapers.length > 0 && qPapers[0] && qPapers[0].course == v_teacher.course) {
                    v_qPaper = qPapers[0];
                    return req.app.get("db").query(sql.select_all_answer_papers_with_Qname, [v_qPaper.filename]);
    ...
    if (req.session.user && req.session.user.type == "teacher") {
        if (vfv.verify_Qname(req.body.Qname) && req.body.marks && req.body.marks >= 0 && req.body.marks <= 9999 && req.files && req.files.Qfile
            ...
            qry.then(([teachers]) => {
                if (teachers.length > 0 && teachers[0]) {
                    v_teacher = teachers[0];
                    return req.app.get("db").query(sql.select_question_papers_with_name, [req.body.Qname]);
                } else redirecth.permission_denied(req, res, null);
                return [null];
            }).then(([qPapers]) => {
                if (!qPapers) return;
                if (qPapers.length == 0)
                    return req.app.get("db").query(sql.insert_question_papers_table, [req.body.Qname, v_teacher.course, req.body.marks]);
            ...
            qry.then(([teachers]) => {
                ...
                .then(([qPapers]) => {
                    ...
                    if (qPapers[0] && qPapers[0].course == v_teacher.course) {
                        v_qPaper = qPapers[0];
                        return req.app.get("db").query(sql.update_question_papers_with_name, [active, v_qPaper.filename]);
                    }
                    else redirecth.not_found(req, res, "question-paper/" + req.body.Qname);
                }).then((result) => {
                    if (!result) return;
                    return req.app.get("db").query(sql.update_all_question_papers_in_course_except_name, [v_qPaper.course, v_qPaper.filename]);
                }).then((result) => {
                    if (!result) return;
...
router.post("/edit-answer-paper", (req, res) => {
    if (req.session.user && req.session.user.type == "teacher" && vfv.verify_Qname(req.body.Qname) && vfv.verify_userid(req.body.userid) && req.body.marks) {
        ...
        qry.then(([teachers]) => {
            ...
            if (!qPapers) return [null];
            if (qPapers.length > 0 && qPapers[0] && qPapers[0].course == v_teacher.course)
                return req.app.get("db").query(sql.select_answer_papers_with_Qname_id, [qPapers[0].filename, req.body.userid]);
            else redirecth.permission_denied(req, res, "question-paper/" + req.body.Qname);
            return [null];
        }).catch(err => redirecth.system_error(req, res, err, "answer-papers"))
            .then(([aPapers]) => {
                if (!aPapers) return;
                if (aPapers.length > 0 && aPapers[0])
                    return req.app.get("db").query(sql.update_answer_papers_table, [req.body.marks, aPapers[0].question_filename, aPapers[0].userid]);
                else redirecth.not_found(req, res, "question-paper/" + req.body.Qname);
            }).then((result) => {
                if (!result) return;
...
router.get("/students", (req, res) => {
    if (req.session.user && req.session.user.type == "teacher") {
        ...
        qry.then(([teachers]) => {
            if (teachers.length > 0 && teachers[0])
                return req.app.get("db").query(sql.select_all_students_with_course, [teachers[0].course]);
            else redirecth.permission_denied(req, res, null);
            return [null];
        }).then(([students]) => {
            if (!students) return;
...
router.get("/exams", (req, res) => {
    if (req.session.user && req.session.user.type == "student") {
        let v_qPaper;
        const qry = req.app.get("db").query(sql.select_students_with_id, [req.session.user.userid]);
        qry.then(([students]) => {
            if (students.length > 0 && students[0])
                return req.app.get("db").query(sql.select_question_papers_with_course_active, [students[0].course]);
            else redirecth.permission_denied(req, res, null);
            return [null];
        }).then(([qPapers]) => {
            if (!qPapers) return [null];
            if (qPapers.length > 0 && qPapers[0]) {
                v_qPaper = qPapers[0];
                return req.app.get("db").query(sql.select_answer_papers_with_Qname_id, [v_qPaper.filename, req.session.user.userid]);
...
router.post("/submit-exam", (req, res) => {
    if (req.session.user && req.session.user.type == "student") {
        if (vfv.verify_Qname(req.body.Qname) && req.files && req.files.Afile && req.files.Afile.mimetype == "application/pdf"
            ...
            req.body.Qname = req.body.Qname.toLowerCase();
            const qry = req.app.get("db").query(sql.select_question_papers_with_name, [req.body.Qname]);
            qry.then(([qPapers]) => {
                if (qPapers.length > 0 && qPapers[0] && qPapers[0].active == true)
                    return req.app.get("db").query(sql.select_answer_papers_with_Qname_id, [qPapers[0].filename, req.session.user.userid]);
                else redirecth.not_found(req, res, "exams");
                return [null];
            }).then(([aPapers]) => {
                if (!aPapers) return;
                if (aPapers.length == 0)
                    return req.app.get("db").query(sql.insert_answer_papers_table, [req.body.Qname, req.session.user.userid]);
...
router.get("/results", (req, res) => {
    if (req.session.user && req.session.user.type == "student") {
        const qry = req.app.get("db").query(sql.select_all_results_with_id, [req.session.user.userid]);
