...
router.get("/question-papers", (req, res) => {
    if (req.session.user && req.session.user.type == "admin") {
        const qry = req.app.get("db").query(sql.select_all_question_papers);
...
router.get("/answer-papers", (req, res) => {
    if (req.session.user && req.session.user.type == "admin") {
        const qry = req.app.get("db").query(sql.select_all_answer_papers,);
        ...
        if (vfv.verify_Qname(req.body.Qname)) {
            ...
            qry.then(([qPapers]) => {
                if (qPapers.length > 0 && qPapers[0])
                    return req.app.get("db").query(sql.select_all_answer_papers_with_Qname, [req.body.Qname]);
...
router.post("/delete-answer-paper", (req, res) => {
    if (req.session.user && req.session.user.type == "admin") {
        if (vfv.verify_Qname(req.body.Qname), vfv.verify_userid(req.body.userid)) {
            ...
            const qry = req.app.get("db").query(sql.select_answer_papers_with_Qname_id, [req.body.Qname, req.body.userid]);
            qry.then(([aPapers]) => {
